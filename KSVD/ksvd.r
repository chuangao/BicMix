 ksvd <- function(inputDir=NULL,inputFileName=NULL,n=NA,p=NA,k=NA, outputDir=NULL,scriptFile=NULL,matlabWhere){

    system(paste0("mkdir -p ",outputDir))

    script <- readLines(scriptFile)

    itemToReplace <- c("inputDirToReplace", "inputFileToReplace", "outputDirToReplace","nrowToReplace","ncolToReplace","FactorNumberToReplace")
    ...
    system(cmd)
    lam <- t(as.matrix(read.table(file.path(outputDir,paste0(inputFileName,".coef")))))
    return(list(lam=lam))
 }
