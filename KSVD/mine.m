...
param.K = 20; % number of dictionary elements
...
fid = fopen(strcat('~/SFA/data/Y_nu_',num2str(nu),'.txt'));
...
dlmwrite(strcat('~/SFA/KSVD/result/Dic_',num2str(nu),'.txt'), Dictionary, 'delimiter', '\t','precision', 6);
dlmwrite(strcat('~/SFA/KSVD/result/Coef_',num2str(nu),'.txt'), full(output.CoefMatrix), 'delimiter', '\t','precision', 6);
