...
for g = 1:rep
   ...
   %%%%%%%%%% Chuan's implementation, select the parameters based on the likelihood.
        ...
        if mod(i,100) == 0
            like=0;
            for  j=1:p
                like = like + sum(log(normpdf(Ytil(:,j),0,sqrt(Sigma(j,j)))));         
            end
            like=like+sum(log(gampdf(diag(Sigma),as,1/bs)));
            like = like + sum(sum(log(normpdf(eta,0,1))));
            like=like+sum(sum(log(gampdf(psijh,df/2,2/df))));
            like=like+log(gampdf(delta(1),ad,1/bd));      
            like=like+sum(log(gampdf(delta(2:k),ad2,1/bd2)));
          
            like=like + sum(sum(log(normpdf(Lambda,0,sqrt(1./Plam)))));        
 %disp(like)
            count=count+1;
            like_v(count)=like;
...
dlmwrite(strcat('Lambda.txt'), lamBak, 'delimiter', '\t','precision', 6);
dlmwrite(strcat('EX.txt'), etaBak, 'delimiter', '\t','precision', 6);
...
for itr=1:1000
        ...
        for j=1:k
          like = like + sum(log(normpdf(eta(:,j),0,1)));
        end
          like=like + sum(sum(log(normpdf(Lambda,0,sqrt(1./Plam)))));        
        for  j=1:k
            for l=1:p
                like=like+log(gampdf(psijh(l,j),df/2,2/df));
            end
            if j==1
                like=like+log(gampdf(delta(j),ad,1/bd));
            else
                 like=like+log(gampdf(delta(j),ad2,1/bd2));
            end
        end
        for j=1:p
            like=like+log(gampdf(Sigma(j,j),as,1/bs));
        end
  end
